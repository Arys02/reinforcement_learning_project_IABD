...
impl<B: burn::prelude::Backend> MyQMLP<B> {
    pub fn new(device: &B::Device, input_state_features: usize, output_actions: usize) -> Self {
        let linear1 = nn::LinearConfig::new(input_state_features, 500)
            .with_bias(true)
            .init(device);
        let linear2 = nn::LinearConfig::new(500, 300).with_bias(true).init(device);
        let linear3 = nn::LinearConfig::new(300, 100).with_bias(true).init(device);
        let output_layer = nn::LinearConfig::new(100, output_actions)
            .with_bias(true)
            .init(device);
        MyQMLP {
            linear1,
