...
impl Environment for SecretEnv0 {
    ...
    fn display(&self) {
        unsafe {
            let secret_env_0_display: libloading::Symbol<unsafe extern fn(*const c_void)> = LIB.get(b"secret_env_0_display").expect("Failed to load function `secret_env_0_display`");
